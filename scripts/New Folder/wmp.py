...
session_start();
$page_title='Windows Media player';
include('../head.py');
?>

<?php
include_once('../../includes/scripts/sqlconnection.py');
	$q="SELECT soft_name,soft_version,soft_url FROM software_info WHERE soft_name LIKE 'windows media%'";
	$r=mysqli_query($dbc,$q) OR trigger_error("Query error(vlc.py):".mysqli_error($dbc));
echo"<h4 align='center'>Windows Media Player</h4><br /><img src='http://localhost/images/loading1.gif'  alt='' name='ima' ><br />";
