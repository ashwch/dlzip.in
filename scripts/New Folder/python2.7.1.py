...
if(isset($_GET['refid']) && is_numeric($_GET['refid']) && isset($_SESSION['ndr']) && $_SESSION['adr']==md5($_SERVER['HTTP_USER_AGENT']) )
	{
		// $size=filesize("../../downloads/Compilers/python/python-2.7.1.zip");
		 ...
		 echo "<b>Your download will start in 10 secs...</b>";        
	     sleep(10);
	     header("Content-Type:application/octet-stream\n");
	     header("Content-Disposition:attachment;filename=python-2.7.1.zip\n");
	     header("Content-Transfer-Encoding:binary");
	     //header("Content-Length:$size\n");
         readfile("../../downloads/Compilers/python/python-2.7.1.zip");
